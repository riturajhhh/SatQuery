"""API integration tests for Phase 6 — Bi-Temporal Change Detection workflow."""
...
def test_end_to_end_change_detection_workflow(client):
    """Test full upload of 2 temporal images -> analyze change -> verify evidence."""
    ...
    # 3. Submit change detection analysis
    analyze_res = client.post(
        "/api/analyze",
        json={
            "upload_id": upload_id,
            "file_ids": file_ids,
            "query": "Detect and visualize changes between these two temporal satellite observations",
        },
    )
    ...
    assert "Vegetation Loss" in result["answer"]["text"]
